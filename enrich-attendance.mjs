/**
 * Enriches data/term_output.json with player-attendance data from
 * the Barca Academy API. Adds to each coach entry:
 ...
 *   data/sessions.csv (same source used by process.py), which has both
 *   training days per week instead of only one.
 *
...
const TOKEN = 'bff40f954954bf2c8fafa4cc1dbb7fe06b14de8afb9c754e19bb9bdcf3b970b5';
const API   = 'https://attendance.barcaacademy.sg/api/attendance/logs';
const FILE  = 'data/term_output.json';
const CSV   = 'data/sessions.csv';

const data      = JSON.parse(readFileSync(FILE, 'utf8'));
const termStart = new Date(data.term.from + 'T00:00:00Z');
const todayStr  = new Date().toISOString().slice(0, 10);
const T2_FROM   = data.term.from;
const T2_TO     = data.term.to;

// ── Parse sessions.csv for actual Select session dates ────────────────────────
...
try {
  ...
  for (let i = 1; i < csvLines.length; i++) {
    ...
    if ((cols[23] || '').trim() !== 'Select') continue;
    const dateStr = csvParseDate(cols[5]);
    if (!dateStr || dateStr < T2_FROM || dateStr > T2_TO) continue;
    if (utcDayOfWeek(dateStr) === 0) continue; // exclude Sundays (matches process.py filter)
    if ((cols[18] || '').trim() === 'No clock in') continue; // same status filter

    const teamKey = (cols[7] || '').trim().toUpperCase();
    const coachId = parseInt((cols[20] || '').trim(), 10);
  ...
  console.log(`CSV loaded: ${selectRows} Select rows → ${selectPresence.size} teams`);
} catch (e) {
...
// Term starts 2026-04-20 (Monday), so offset 0 = Monday of each week.
...
for (const cat of data.categories) {
  for (const sess of cat.sessions) {
    ...
    if (dayOffset !== null) {
      ...
      // Select: use actual dates from sessions.csv
      ...
      if (selDates) {
        ...
        // CSV not available → fall back to Saturday per active week
        const refByWeek = sess.coaches?.[0]?.by_week || [];
        refByWeek.forEach((val, wIdx) => {
          if (val !== null && val !== undefined) {
...
for (const cat of data.categories) {
  for (const sess of cat.sessions) {
    ...
    for (const coach of sess.coaches || []) {
      ...
      if (isSelect) {
        // Select: presence data comes directly from sessions.csv
        for (const dateStr of dates) {
          if (!teamPresence || !teamPresence.has(dateStr)) {
            // No session on this date for this Select team
            by_date.push(null);
        ...
        for (const dateStr of dates) {
          ...
          if (dayOfWeekInTerm !== dayOffset || wIdx < 0 || wIdx >= 12) {
            by_date.push(null);
            att_by_date.push(null);
            continue;
          }
